...
This branch now directly consumes the public models, current multi-pass engine,
strict ECG layout parser and analyzer lifecycle port. It is not the completed
canonical-contract/plugin extraction or a new binary release. Previous sealed GUI
cohorts and separately verified binaries remain unchanged.
...
The current engine was extracted from `e1ef13d`, not replaced by the older public
snapshot. App `application/multi_pass.py`, `domain/ekg_layout.py` and the duplicate
`VisionAnalyzerService` definition are removed, without forwarding modules. Both
desktop and evaluation entrypoints import public implementations directly and
inject the existing App checklist and OpenClaw stage identities. Current prompts,
...
Further stages must assemble the canonical evidence contract, bind the OpenClaw plugin and external

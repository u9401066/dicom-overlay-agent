...
Adding a new imaging modality (KUB, echo, CT, MRI, ...) used to mean editing
seven scattered places (the ``Modality`` enum, the OpenClaw skill map, the skill
path map, the validator checklist table, the input-guard supported set, the
overlay icon map, and the modality cycle in ``__main__``). This module collapses
all of that per-modality knowledge into one :class:`ModalityProfile` and a
...
class ModalityProfile:
    ...
    icon: str = "📊"
    """Emoji/icon shown in the overlay summary panel."""

    skill_name: str = ""
    """OpenClaw workspace skill folder; defaults to ``dicom-<key>-analysis``."""

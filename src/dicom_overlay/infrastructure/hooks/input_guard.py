...
_SUPPORTED_MODALITIES = frozenset({Modality.EKG, Modality.CXR, Modality.CT_BRAIN})
...
class InputGuard(AnalyzeHook):
    ...
    def pre_analyze(self, request: AnalyzeRequest) -> AnalyzeRequest:
        ...
        if request.modality not in _SUPPORTED_MODALITIES:
            raise HookError(
                f"不支援的影像模態: {request.modality.value}, "
                f"支援: {', '.join(m.value for m in _SUPPORTED_MODALITIES)}"
            )

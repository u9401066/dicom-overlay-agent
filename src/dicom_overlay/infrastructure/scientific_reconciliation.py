...
def reconciliation_instruction() -> str:
    return (
        ...
        "decision across the whole linked claim graph: an unchanged finding "
        "label/detail is not enough for confirm. All its referenced observations "
        "must preserve anatomy, finding, polarity, status, claim_type, assessable, "
        "laterality, temporal and question exactly; only evidence_ids may change. "
        "Shortening a sentence, removing a negative clause, or renaming anatomy "
        "is a revision even when the headline conclusion stays the same. If a "
        "shared observation changes, use revise for every retained finding "
        "referencing it. Make corrections when needed, do not preserve an error "
        "just to use confirm. Explain each "
